...
// shift for HAL, FMC, FreeRTOS
    ...
    rtcTaskHandle = osThreadNew(RTCTask, (void*)1, &rtcTask_attributes);

    /* creation of communication task */
    comTaskHandle = osThreadNew(COMTask, (void*)1, &comTask_attributes);

    xQueueRX = xQueueCreate(1, sizeof(msgData));
...
__attribute__((unused)) static void RTCTask(void *params)
{
    ...
    for (;;)
    {
        ...
        if (xQueueRX != 0)
        {
          if (xQueueReceive(xQueueRX, &msgOSRX, 0) == pdPASS)
          {         
            if(msgOSRX.id == 1 && msgOSRX.data[0] == 0)
            {          
              if(vTaskDelete != NULL)
              {
            ...
            else if(msgOSRX.id == 1 && msgOSRX.data[0] == 100)
            {
...
__attribute__((unused)) static void COMTask( void* params )
{
