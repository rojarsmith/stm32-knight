...
ContentE4::ContentE4()	
	:
	textarea_move_ended_callback_(this, &ContentE4::textareaMoveEndedHandler),
    textarea_fade_ended_callback_(this, &ContentE4::textareaFadeEndedHandler),
	textareawithtwowildcards_fade_ended_callback_(this, &ContentE4::textareawithtwowildcardsFadeEndedHandler),
	buttonwithlabel_move_ended_callback_(this, &ContentE4::buttonwithlabelMoveEndedHandler),
	buttonwithlabel_fade_ended_callback_(this, &ContentE4::buttonwithlabelFadeEndedHandler),
	button_clicked_(this, &ContentE4::buttonClickedHandler)
{
}
...
void ContentE4::handleTickEvent()
{
	AbstractContent::handleTickEvent();
}

void ContentE4::initialize(MachineStatus *status)
{	
	AbstractContent::initialize(status);

	title_.setTypedText(TypedText(T_E4_TITLE));
	...
	//body_.setTypedText(TypedText(T_E4_BODY));
...
void ContentE4::textareaMoveEndedHandler(const MoveAnimator<FadeAnimator<TextArea>>& textarea)
{
	animation_lock_--;
}

void ContentE4::textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea)
{
	animation_lock_--;
}

void ContentE4::textareawithtwowildcardsFadeEndedHandler(const FadeAnimator<TextAreaWithTwoWildcards>& textarea)
{
	animation_lock_--;
}

void ContentE4::buttonwithlabelMoveEndedHandler(const MoveAnimator<FadeAnimator<ButtonWithLabel>>& button)
{
	animation_lock_--;
}

void ContentE4::buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& button)
{
	animation_lock_--;
}

void ContentE4::buttonClickedHandler(const AbstractButton& source)
{
	if (&ok_ == &source)
	{
...
void ContentE4::event5()
{
	AbstractContent::event5();
}

void ContentE4::event6()
{
