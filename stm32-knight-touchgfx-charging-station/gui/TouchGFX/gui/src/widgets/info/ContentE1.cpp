...
ContentE1::ContentE1()	
	:
	textarea_move_ended_callback_(this, &ContentE1::textareaMoveEndedHandler),
    textarea_fade_ended_callback_(this, &ContentE1::textareaFadeEndedHandler),
	textareawithtwowildcards_fade_ended_callback_(this, &ContentE1::textareawithtwowildcardsFadeEndedHandler),
	buttonwithlabel_move_ended_callback_(this, &ContentE1::buttonwithlabelMoveEndedHandler),
	buttonwithlabel_fade_ended_callback_(this, &ContentE1::buttonwithlabelFadeEndedHandler),
	button_clicked_(this, &ContentE1::buttonClickedHandler)
{
}
...
void ContentE1::handleTickEvent()
{
	AbstractContent::handleTickEvent();
}

void ContentE1::initialize(MachineStatus *status)
{	
	AbstractContent::initialize(status);

	title_.setTypedText(TypedText(T_E1_TITLE));
	...
	body_.setTypedText(TypedText(T_E1_BODY));
...
void ContentE1::textareaMoveEndedHandler(const MoveAnimator<FadeAnimator<TextArea>>& textarea)
{
	animation_lock_--;
}

void ContentE1::textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea)
{
	animation_lock_--;
}

void ContentE1::textareawithtwowildcardsFadeEndedHandler(const FadeAnimator<TextAreaWithTwoWildcards>& textarea)
{
	animation_lock_--;
}

void ContentE1::buttonwithlabelMoveEndedHandler(const MoveAnimator<FadeAnimator<ButtonWithLabel>>& button)
{
	animation_lock_--;
}

void ContentE1::buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& button)
{
	animation_lock_--;
}

void ContentE1::buttonClickedHandler(const AbstractButton& source)
{
	if (&ok_ == &source)
	{
...
void ContentE1::event5()
{
	AbstractContent::event5();
}

void ContentE1::event6()
{
