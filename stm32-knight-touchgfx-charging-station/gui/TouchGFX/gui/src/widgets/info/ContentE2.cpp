...
ContentE2::ContentE2()	
	:
	textarea_move_ended_callback_(this, &ContentE2::textareaMoveEndedHandler),
    textarea_fade_ended_callback_(this, &ContentE2::textareaFadeEndedHandler),
	buttonwithlabel_move_ended_callback_(this, &ContentE2::buttonwithlabelMoveEndedHandler),
	buttonwithlabel_fade_ended_callback_(this, &ContentE2::buttonwithlabelFadeEndedHandler),
	button_clicked_(this, &ContentE2::buttonClickedHandler)
{
}
...
void ContentE2::handleTickEvent()
{
	AbstractContent::handleTickEvent();
}

void ContentE2::initialize(MachineStatus *status)
{	
	AbstractContent::initialize(status);

	title_.setTypedText(TypedText(T_E2_TITLE));
	...
	body_.setTypedText(TypedText(T_E2_BODY));
...
void ContentE2::textareaMoveEndedHandler(const MoveAnimator<FadeAnimator<TextArea>>& textarea)
{
	animation_lock_--;
}

void ContentE2::textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea)
{
	animation_lock_--;
}

void ContentE2::buttonwithlabelMoveEndedHandler(const MoveAnimator<FadeAnimator<ButtonWithLabel>>& button)
{
	animation_lock_--;
}

void ContentE2::buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& button)
{
	animation_lock_--;
}

void ContentE2::buttonClickedHandler(const AbstractButton& source)
{
	if (&ok_ == &source)
	{
...
void ContentE2::event5()
{
	AbstractContent::event5();
}

void ContentE2::event6()
{
