...
Content0504::Content0504()	
	:
    textarea_fade_ended_callback_(this, &Content0504::textareaFadeEndedHandler),
	buttonwithlabel_fade_ended_callback_(this, &Content0504::buttonwithlabelFadeEndedHandler),
	image_fade_ended_callback_(this, &Content0504::imageFadeEndedHandler),
	button_clicked_(this, &Content0504::buttonClickedHandler)
{
}
...
void Content0504::handleTickEvent()
{
	AbstractContent::handleTickEvent();
}

void Content0504::initialize(MachineStatus *status)
{	
	AbstractContent::initialize(status);

	title_.setTypedText(TypedText(T_P0504_TITLE));
	...
	body_.setTypedText(TypedText(T_P0504_BODY));
...
void Content0504::textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea)
{
	animation_lock_--;
}

void Content0504::buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& button)
{
	animation_lock_--;
}

void Content0504::imageFadeEndedHandler(const FadeAnimator<Image>& image)
{
	animation_lock_--;
}

void Content0504::buttonClickedHandler(const AbstractButton& source)
{
	if (&ok_ == &source)
	{
...
void Content0504::eventTriggerHandler(const int source)
{
...
void Content0504::event3()
{
	ok_.setTouchable(true);

	AbstractContent::event3();

	//em_.addCountDownEvent(6, 600);
	em_.addCountDownEvent(10, 180);
}

void Content0504::event4()
{
	animation_lock_ = 3;
...
void Content0504::event5()
{
	AbstractContent::event5();
}

void Content0504::event6()
{
