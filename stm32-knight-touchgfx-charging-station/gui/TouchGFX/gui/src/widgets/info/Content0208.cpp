...
Content0208::Content0208()
	:
    textarea_fade_ended_callback_(this, &Content0208::textareaFadeEndedHandler),
	textareawithonewildcard_fade_ended_callback_(this, &Content0208::textareawithonewildcardFadeEndedHandler),
	buttonwithlabel_fade_ended_callback_(this, &Content0208::buttonwithlabelFadeEndedHandler),
	image_fade_ended_callback_(this, &Content0208::imageFadeEndedHandler),
	button_clicked_(this, &Content0208::buttonClickedHandler)
{
}
...
void Content0208::handleTickEvent()
{
	AbstractContent::handleTickEvent();
}

void Content0208::initialize(MachineStatus *status)
{
	...
	bg2_.setColor(Color::getColorFromRGB(255, 255, 255));
	bg2_.setPosition(50, 433, 538, 243);
	bg2_.setAlpha(0);
	add(bg2_);

	body_.setTypedText(TypedText(T_P0208_TFIT1));
	body_.setColor(Color::getColorFromRGB(255, 255, 255));
	body_.setPosition(97, 480, 334, 190);
	body_.setAlpha(0);
	body_.setWideTextAction(WIDE_TEXT_WORDWRAP);
	add(body_);

	body_image_.setBitmap(Bitmap(BITMAP_PAYMENT_QR_ID));
	body_image_.setXY(466, 518);
	body_image_.setAlpha(0);
	add(body_image_);
...
void Content0208::textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea)
{
	animation_lock_--;
}

void Content0208::textareawithonewildcardFadeEndedHandler(const FadeAnimator<TextAreaWithOneWildcard>& textarea)
{
	animation_lock_--;
}

void Content0208::buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& button)
{
	animation_lock_--;
}

void Content0208::imageFadeEndedHandler(const FadeAnimator<Image>& image)
{
	animation_lock_--;
}

void Content0208::buttonClickedHandler(const AbstractButton& source)
{
	if (&ok_ == &source)
	{
...
void Content0208::eventTriggerHandler(const int source)
{
...
void Content0208::event2()
{
	...
	//bg2_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	bg2_.startFadeAnimation(51, animation_duration_);
	//body_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	body_.startFadeAnimation(255, animation_duration_);
	//body_image_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	body_image_.startFadeAnimation(255, animation_duration_);
...
void Content0208::event4()
{
	...
	//bg2_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	bg2_.startFadeAnimation(0, animation_duration_);
	//body_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	body_.startFadeAnimation(0, animation_duration_);
	//body_image_.setFadeAnimationEndedAction(textareawithonewildcard_fade_ended_callback_);
	body_image_.startFadeAnimation(0, animation_duration_);
	ok_.setFadeAnimationEndedAction(buttonwithlabel_fade_ended_callback_);
...
void Content0208::event5()
{
	AbstractContent::event5();
}

void Content0208::event6()
{
