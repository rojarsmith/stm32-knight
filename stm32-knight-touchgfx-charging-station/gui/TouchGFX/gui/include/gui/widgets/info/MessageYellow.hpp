#ifndef MESSAGEYELLOW_HPP
#define MESSAGEYELLOW_HPP
...
class MessageYellow : public AbstractMessage
{
public:
	MessageYellow();
	virtual ~MessageYellow();
...
private:
	...
	Callback<MessageYellow, const FadeAnimator<Box>& > box_fade_ended_callback_;
	void boxFadeEndedHandler(const FadeAnimator<Box>& box);
	Callback<MessageYellow, const MoveAnimator<FadeAnimator<Box>>& > box_move_ended_callback_;
	void boxMoveEndedHandler(const MoveAnimator<FadeAnimator<Box>>& box);

	Callback<MessageYellow, const MoveAnimator<FadeAnimator<Image>>& > image_move_ended_callback_;
	void imageMoveEndedHandler(const MoveAnimator<FadeAnimator<Image>>& image);
	Callback<MessageYellow, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<MessageYellow, const MoveAnimator<FadeAnimator<TiledImage>>& > tiledimage_move_ended_callback_;
	void tiledimageMoveEndedHandler(const MoveAnimator<FadeAnimator<TiledImage>>& tiledimage);
	Callback<MessageYellow, const FadeAnimator<TiledImage>& > tiledimage_fade_ended_callback_;
