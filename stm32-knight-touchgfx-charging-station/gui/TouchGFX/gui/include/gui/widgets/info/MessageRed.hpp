#ifndef MESSAGERED_HPP
#define MESSAGERED_HPP
...
class MessageRed : public AbstractMessage
{
public:
	MessageRed();
	virtual ~MessageRed();
...
private:
	...
	Callback<MessageRed, const FadeAnimator<Box>& > box_fade_ended_callback_;
	void boxFadeEndedHandler(const FadeAnimator<Box>& box);
	Callback<MessageRed, const MoveAnimator<FadeAnimator<Box>>& > box_move_ended_callback_;
	void boxMoveEndedHandler(const MoveAnimator<FadeAnimator<Box>>& box);

	Callback<MessageRed, const MoveAnimator<FadeAnimator<Image>>& > image_move_ended_callback_;
	void imageMoveEndedHandler(const MoveAnimator<FadeAnimator<Image>>& image);
	Callback<MessageRed, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<MessageRed, const MoveAnimator<FadeAnimator<TiledImage>>& > tiledimage_move_ended_callback_;
	void tiledimageMoveEndedHandler(const MoveAnimator<FadeAnimator<TiledImage>>& tiledimage);
	Callback<MessageRed, const FadeAnimator<TiledImage>& > tiledimage_fade_ended_callback_;
