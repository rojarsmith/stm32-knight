#ifndef CONTENT0504_HPP
#define CONTENT0504_HPP
...
class Content0504 : public AbstractContent
{
public:
	Content0504();
	virtual ~Content0504();
...
private:
	...
	Callback<Content0504, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0504, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0504, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0504, const AbstractButton&>button_clicked_;
