#ifndef CONTENT0203_HPP
#define CONTENT0203_HPP
...
class Content0203 : public AbstractContent
{
public:
	Content0203();
	virtual ~Content0203();
...
private:
	...
	Callback<Content0203, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0203, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0203, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0203, const AbstractButton&>button_clicked_;
