#ifndef CONTENTE4_HPP
#define CONTENTE4_HPP
...
class ContentE4 : public AbstractContent
{
public:
	ContentE4();
	virtual ~ContentE4();
...
private:		
	//static const uint16_t TEXT_SIZE_10 = 10;
	...
	Callback<ContentE4, const MoveAnimator<FadeAnimator<TextArea>>& > textarea_move_ended_callback_;
	void textareaMoveEndedHandler(const MoveAnimator<FadeAnimator<TextArea>>& textarea);

	Callback<ContentE4, const FadeAnimator<TextAreaWithTwoWildcards>& > textareawithtwowildcards_fade_ended_callback_;
	void textareawithtwowildcardsFadeEndedHandler(const FadeAnimator<TextAreaWithTwoWildcards>& textarea);

	Callback<ContentE4, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<ContentE4, const MoveAnimator<FadeAnimator<ButtonWithLabel>>& > buttonwithlabel_move_ended_callback_;
	void buttonwithlabelMoveEndedHandler(const MoveAnimator<FadeAnimator<ButtonWithLabel>>& buttonwithlabel);

	Callback<ContentE4, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<ContentE4, const AbstractButton&>button_clicked_;
