#ifndef CONTENT0404_HPP
#define CONTENT0404_HPP
...
class Content0404 : public AbstractContent
{
public:
	Content0404();
	virtual ~Content0404();
...
private:
	...
	Callback<Content0404, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0404, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0404, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0404, const AbstractButton&>button_clicked_;
