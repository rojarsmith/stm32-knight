#ifndef CONTENT0302_HPP
#define CONTENT0302_HPP
...
class Content0302 : public AbstractContent
{
public:
	Content0302();
	virtual ~Content0302();
...
private:		
	...
	Callback<Content0302, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0302, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0302, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0302, const AbstractButton&>button_clicked_;
