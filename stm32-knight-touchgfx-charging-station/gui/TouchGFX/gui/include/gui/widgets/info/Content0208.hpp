#ifndef CONTENT0208_HPP
#define CONTENT0208_HPP
...
class Content0208 : public AbstractContent
{
public:
	Content0208();
	virtual ~Content0208();
...
private:
	...
	Unicode::UnicodeChar bc_buffer_[TEXT_SIZE_3];

	Callback<Content0208, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0208, const FadeAnimator<TextAreaWithOneWildcard>& > textareawithonewildcard_fade_ended_callback_;
	void textareawithonewildcardFadeEndedHandler(const FadeAnimator<TextAreaWithOneWildcard>& textarea);

	Callback<Content0208, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0208, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0208, const AbstractButton&>button_clicked_;
