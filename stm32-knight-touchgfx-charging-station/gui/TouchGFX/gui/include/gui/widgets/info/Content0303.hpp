#ifndef CONTENT0303_HPP
#define CONTENT0303_HPP
...
class Content0303 : public AbstractContent
{
public:
	Content0303();
	virtual ~Content0303();
...
private:		
	...
	Callback<Content0303, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0303, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0303, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0303, const AbstractButton&>button_clicked_;
