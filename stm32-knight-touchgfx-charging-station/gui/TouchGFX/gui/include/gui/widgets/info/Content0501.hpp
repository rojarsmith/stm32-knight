#ifndef CONTENT0501_HPP
#define CONTENT0501_HPP
...
class Content0501 : public AbstractContent
{
public:
	Content0501();
	virtual ~Content0501();
...
private:
	...
	FadeAnimator<ButtonWithLabel> no_;
	//Container viewport_;
	...
	Callback<Content0501, const FadeAnimator<TextArea>& > textarea_fade_ended_callback_;
	void textareaFadeEndedHandler(const FadeAnimator<TextArea>& textarea);

	Callback<Content0501, const FadeAnimator<ButtonWithLabel>& > buttonwithlabel_fade_ended_callback_;
	void buttonwithlabelFadeEndedHandler(const FadeAnimator<ButtonWithLabel>& buttonwithlabel);

	Callback<Content0501, const FadeAnimator<Image>& > image_fade_ended_callback_;
	void imageFadeEndedHandler(const FadeAnimator<Image>& image);

	Callback<Content0501, const AbstractButton&>button_clicked_;
