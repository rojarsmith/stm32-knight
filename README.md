...
Open source and team-friendly STM32 development templates and practical examples.
...
STM32 VS Code Extension→Import CMake project→Select Board→Actions→Open in this window
...
CMake→PROJECT STATUS→Configure: local-debug→Build: local-debug

## Debug

Project Outline→CMakeLists.txt at root: Configure with CMake Debugger→Select and Start Debug Configuration→Debug CM7 - ST-Link
...
Q: Not found binary or `"executable": "${command:STM32VSCodeExtension.dualCoreTarget}"` not define ?

A: `.vscode/launch.json` modify:

```json
"configurations": [{
    "executable": "${workspaceFolder}/build/core0/build/stm32-knight-xxx.elf",
}]
```

Q: Compile after change assets not work ?

A: Delete build.

## Project Directory

### stm32-knight-clean-board

The scripts that resets the hardware environment on the development board, such as clearing the internal and external ROM.
...
![stm32-knight-touchgfx-3d-gesture-game-01](img/stm32-knight-touchgfx-3d-gesture-game-01.png)![stm32-knight-touchgfx-3d-gesture-game-02](img/stm32-knight-touchgfx-3d-gesture-game-02.png)![stm32-knight-touchgfx-3d-gesture-game-03](img/stm32-knight-touchgfx-3d-gesture-game-03.png)

Communicate files:

communicate\grpc\generate-code.bat

communicate\grpc\sensor.proto

communicate\grpc\sensor_pb2_grpc-test-client.py

```shell
# C:\my\toolchain\vcpkg-2025.04.09
bootstrap-vcpkg.bat
vcpkg integrate install
vcpkg integrate project
Install-Package "vcpkg.C.my.toolchain.vcpkg2025.04.09" -Source "C:\my\toolchain\vcpkg-2025.04.09"

Use Vcpkg: True
Install Vcpkg Dependencies: True
Use AutoLink: True

# TouchGFX only support 32bit
vcpkg install grpc:x86-windows-release
vcpkg install grpc:x64-windows-release
