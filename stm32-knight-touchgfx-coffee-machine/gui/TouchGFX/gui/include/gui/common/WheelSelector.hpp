...
//    virtual void setup(int width, int height, int textXPosition, int textYPosition, TEXTS wheelText);
    virtual void setup(int width, int height, int imgXPosition, int imgYPosition, const uint16_t img_id); // @param wheelText : The wheel image.
...
//    void setTextColor(int16_t normalColor, int16_t selectedColor, uint16_t selectedBackgroundColor, int selectedYOffset = 0, int selectedHeight = 0);
     ...
     */
    void reset();
    bool getWheelDragStatus();
    void setWheelDragStatus();
    ...
    void setSlider(Slider &waterSlider, Slider &beanSlider, Slider &milkSlider, Slider &steamSlider, Slider &sugarSlider)
    {
        this->waterSlider = &waterSlider;
        this->beanSlider = &beanSlider;
        this->milkSlider = &milkSlider;
        this->steamSlider = &steamSlider;
        this->sugarSlider = &sugarSlider;
    }
    void lockSlider()
    {
        this->waterSlider->setTouchable(false);
        this->beanSlider->setTouchable(false);
        this->milkSlider->setTouchable(false);
        this->steamSlider->setTouchable(false);
        this->sugarSlider->setTouchable(false);
    }
...
//    int selectedTextYPosition;
    int selectedImageYPosition;
//    int spaceBetweenTextRows;
    int item_height; // 1 item of images height
    int CutOffValue;
    int topCutOff;
    int bottomCutOff;
...
//    void textMoveEndedHandler(const MoveAnimator<TextArea>& text);
    Callback<WheelSelector, const MoveAnimator<Image>& > imageMoveEndedCallback;
    void imageMoveEndedHandler(const MoveAnimator<Image>& image);

//    virtual void moveText(int newYPosition);
    virtual void moveImage(int newYPosition);
//    virtual void animateMoveText(int newYPosition, int duration, EasingEquation equation);
    virtual void animateMoveImage(int newYPosition, int duration, EasingEquation equation);
