...
TemperatureScrollingWheel::TemperatureScrollingWheel() : ScrollingWheel()

{
    ...
    for (int i = 0; i < NUMBER_OF_ITEMS; i++)
    {
        ...
        backgroundScrollingMenuItems.element[i].setTypedText(TypedText(T_TEMPERATURE_VALUE));
        ...
        centerScrollingMenuItems.element[i].setTypedText(TypedText(T_TEMPERATURE_VALUE_CENTER));
    }
}

void TemperatureScrollingWheel::initializeBackgroundScrollingMenuItem(int containerIndex, int itemIndex)
{
    Unicode::snprintf(backgroundScrollingMenuItems.element[containerIndex].textBuffer,
        backgroundScrollingMenuItems.element[containerIndex].TEXT_SIZE, "%i", 300 - (itemIndex * 25));
}

void TemperatureScrollingWheel::initializeCenterScrollingMenuItem(int containerIndex, int itemIndex)
{
    Unicode::snprintf(centerScrollingMenuItems.element[containerIndex].textBuffer,
        centerScrollingMenuItems.element[containerIndex].TEXT_SIZE, "%i", 300 - (itemIndex * 25));
}
